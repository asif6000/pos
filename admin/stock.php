...
if ($isGlobalView) {
    // Global Stats
    $lowStockCount = $db->query("SELECT COUNT(*) FROM (SELECT p.id, SUM(ss.quantity) as stock, p.min_stock FROM products p LEFT JOIN store_stocks ss ON p.id = ss.product_id WHERE p.status = 'active' GROUP BY p.id HAVING stock <= min_stock) as t")->fetchColumn();
    $outOfStockCount = $db->query("SELECT COUNT(*) FROM (SELECT p.id, SUM(ss.quantity) as stock FROM products p LEFT JOIN store_stocks ss ON p.id = ss.product_id WHERE p.status = 'active' GROUP BY p.id HAVING stock = 0 OR stock IS NULL) as t")->fetchColumn(); // Simplified logic
    $totalStockValue = $db->query("SELECT SUM(stock * buy_price) FROM (SELECT p.buy_price, COALESCE(SUM(ss.quantity), 0) as stock FROM products p LEFT JOIN store_stocks ss ON p.id = ss.product_id WHERE p.status = 'active' GROUP BY p.id) as t")->fetchColumn();
} else {
    // Store Stats
