...
// Show welcome message when redirected from subscription approval
if (isset($_GET['approved']) && $_GET['approved'] == '1') {
    setFlash('success', '🎉 আপনার Subscription সক্রিয় হয়েছে! এখন আপনার Plan এর সব features ব্যবহার করতে পারবেন।');
}
...
// Current subscription / plan for this owner (drives module access)
$currentSub = null;
try {
    $stmt = $db->prepare("
...
try {
    ...
    // ── Today's Discounts ─────────────────────────────────────────────────
    list($sql, $params) = scopeSql("SELECT COALESCE(SUM(s.discount_amount), 0) as discount, COALESCE(SUM(s.subtotal), 0) as subtotal FROM sales s", $store_id, $owner_id);
    $sql .= " AND s.created_at BETWEEN ? AND ?";
    $params[] = $todayStart; $params[] = $todayEnd;
    $stmt = $db->prepare($sql); $stmt->execute($params);
    ...
    // ── All-time Discounts ────────────────────────────────────────────────
    list($sql, $params) = scopeSql("SELECT COALESCE(SUM(s.discount_amount), 0) as discount, COALESCE(SUM(s.subtotal), 0) as subtotal, COALESCE(SUM(s.total), 0) as total FROM sales s", $store_id, $owner_id);
    $stmt = $db->prepare($sql); $stmt->execute($params);
    ...
    $params = [];
    $sql = "SELECT st.id as store_id, st.name as store_name, COUNT(s.id) as sale_count, COALESCE(SUM(s.total), 0) as total, COALESCE(SUM(s.discount_amount), 0) as discount
            FROM stores st
    ...
    $sql = "SELECT st.id as store_id, st.name as store_name,
            COUNT(s.id) as sale_count, COALESCE(SUM(s.total), 0) as total, COALESCE(SUM(s.discount_amount), 0) as discount
            FROM stores st
            LEFT JOIN users u ON u.store_id = st.id
            LEFT JOIN sales s ON s.user_id = u.id
            WHERE 1=1";
    ...
    if ($store_id) {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM (SELECT p.id, COALESCE(SUM(ss.quantity), 0) as qty FROM products p LEFT JOIN store_stocks ss ON p.id = ss.product_id WHERE p.status = 'active' GROUP BY p.id, p.min_stock HAVING qty > 0 AND qty <= p.min_stock) t");
        $p = [];
        $stmt->execute($p);
        $lowStockCount = $stmt->fetch()['count'];
        $stmt = $db->prepare("SELECT p.id, p.name, COALESCE(SUM(ss.quantity), 0) as stock, p.min_stock
                                FROM products p
                                LEFT JOIN store_stocks ss ON p.id = ss.product_id
                                WHERE p.status = 'active'" . (isSuperAdmin() ? "" : " AND p.owner_id = ?") . "
                                GROUP BY p.id, p.name, p.min_stock
                                HAVING stock > 0 AND stock <= p.min_stock
                                ORDER BY stock ASC LIMIT 6");
        $p = isSuperAdmin() ? [] : [$owner_id];
        $stmt->execute($p);
        $lowStockProducts = $stmt->fetchAll();
    }
    ...
    $cashMonth = $stmt->fetch();

    $todayProfit = (float)$todaySales['total'] - (float)$cashToday['cash_out'];
    $monthlyProfit = (float)$monthlySales['total'] - (float)$cashMonth['cash_out'];
    $allTimeProfit = (float)$allTimeSales['total'] - (float)$cashTotals['cash_out'];
...
<!-- Current Plan Banner (hidden for Super Admin) -->
<?php if (!$isSuper): ?>
<?php if ($currentSub): ?>
    <?php
    $subActive  = $currentSub['status'] === 'active' && ($currentSub['end_date'] === null || $currentSub['end_date'] >= date('Y-m-d'));
    $subExpired = !$subActive;
    $daysLeft   = $currentSub['end_date'] ? max(0, (int)ceil((strtotime($currentSub['end_date']) - time()) / 86400)) : null;
    $urgentClass = ($daysLeft !== null && $daysLeft <= 7) ? 'background:linear-gradient(135deg,#7f1d1d,#dc2626)' : 'background:linear-gradient(135deg,#1e1b4b,#4f46e5)';
    ?>
    <div style="<?php echo $subExpired ? 'background:linear-gradient(135deg,#78350f,#d97706)' : $urgentClass; ?>; color:#fff; border-radius:1rem; padding:1rem 1.25rem; margin-bottom:1.5rem; display:flex; align-items:center; gap:1rem; flex-wrap:wrap;">
        <div style="font-size:1.4rem;"><i class="fas fa-<?php echo $subExpired ? 'exclamation-triangle' : 'crown'; ?>"></i></div>
        <div style="flex:1;">
            <div style="font-weight:800;">
                <?php if ($subExpired): ?>
                    Plan Expired: <?php echo sanitize($currentSub['plan_name']); ?>
                <?php else: ?>
                    Current Plan: <?php echo sanitize($currentSub['plan_name']); ?>
                <?php endif; ?>
            </div>
            <div style="font-size:0.8rem; opacity:0.9;">
                <?php if ($subExpired): ?>
                    Your subscription expired on <?php echo date('d M Y', strtotime($currentSub['end_date'])); ?>.
                    Please renew to restore access.
                <?php elseif ($daysLeft !== null && $daysLeft <= 7): ?>
                    <strong>Expiring in <?php echo $daysLeft; ?> day<?php echo $daysLeft !== 1 ? 's' : ''; ?>!</strong>
                    Renew now to avoid interruption.
                <?php elseif ($currentSub['end_date']): ?>
                    Active until <?php echo date('d M Y', strtotime($currentSub['end_date'])); ?>
                <?php else: ?>
                    Active subscription
                <?php endif; ?>
            </div>
        </div>
        <a href="subscription.php" style="color:#fff; text-decoration:none; background:rgba(255,255,255,0.15); padding:0.5rem 1rem; border-radius:999px; font-size:0.85rem; font-weight:600;">
            <?php echo $subExpired ? 'Renew Now' : 'Manage'; ?>
        </a>
        <?php if (hasPermission('plans')): ?>
        <a href="plans.php" style="color:#fff; text-decoration:none; background:rgba(255,255,255,0.15); padding:0.5rem 1rem; border-radius:999px; font-size:0.85rem; font-weight:600;">Manage Plans</a>
        <?php endif; ?>
    </div>
<?php else: ?>
    <div style="background:#fef2f2; border:1px solid #fecaca; border-radius:1rem; padding:0.9rem 1.25rem; margin-bottom:1.5rem; display:flex; align-items:center; gap:1rem;">
        <div style="font-size:1.2rem; color:#dc2626;"><i class="fas fa-exclamation-circle"></i></div>
        <div style="flex:1; font-size:0.9rem; color:#7f1d1d;">
            <strong>No active subscription.</strong>
            You need a subscription plan to access POS features.
            <a href="subscription.php" style="color:#dc2626; font-weight:700;">Purchase a plan</a>.
        </div>
    </div>
<?php endif; ?>
<?php endif; ?>
...
<div class="dash-hero">
    ...
    <div class="dash-hero-bottom">
        ...
        <div class="hero-kpi">
            <div class="hero-kpi-label"><i class="fas fa-tags"></i> Today's Discount</div>
            <div class="hero-kpi-value" style="color:#f87171;"><?php echo formatCurrency($todayDiscount['discount']); ?></div>
            <div class="hero-kpi-sub">Subtotal: <?php echo formatCurrency($todayDiscount['subtotal']); ?></div>
        </div>
        <div class="hero-kpi">
            <div class="hero-kpi-label"><i class="fas fa-percentage"></i> Monthly Discount</div>
            <div class="hero-kpi-value" style="color:#fb923c;"><?php echo formatCurrency($monthlyDiscount['discount']); ?></div>
            <div class="hero-kpi-sub">Subtotal: <?php echo formatCurrency($monthlyDiscount['subtotal']); ?></div>
        </div>
        <div class="hero-kpi">
        ...
        <div class="hero-kpi">
            <div class="hero-kpi-label"><i class="fas fa-coins"></i> Today's Profit</div>
            <div class="hero-kpi-value" style="color:<?php echo $todayProfit >= 0 ? '#4ade80' : '#f87171'; ?>;"><?php echo formatCurrency($todayProfit); ?></div>
            <div class="hero-kpi-sub">Sales: <?php echo formatCurrency($todaySales['total']); ?> · Expenses: <?php echo formatCurrency($cashToday['cash_out']); ?></div>
        </div>
        <div class="hero-kpi">
            <div class="hero-kpi-label"><i class="fas fa-chart-pie"></i> Monthly Profit</div>
            <div class="hero-kpi-value" style="color:<?php echo $monthlyProfit >= 0 ? '#4ade80' : '#f87171'; ?>;"><?php echo formatCurrency($monthlyProfit); ?></div>
            <div class="hero-kpi-sub">Sales: <?php echo formatCurrency($monthlySales['total']); ?> · Expenses: <?php echo formatCurrency($cashMonth['cash_out']); ?></div>
        </div>
        <div class="hero-kpi">
            <div class="hero-kpi-label"><i class="fas fa-piggy-bank"></i> All-Time Profit</div>
            <div class="hero-kpi-value" style="color:<?php echo $allTimeProfit >= 0 ? '#4ade80' : '#f87171'; ?>;"><?php echo formatCurrency($allTimeProfit); ?></div>
            <div class="hero-kpi-sub">Total sales minus total expenses</div>
        </div>
    </div>
</div>
...
<div class="m-stats">
    <div class="m-stat">
        <div class="m-icon rose"><i class="fas fa-tags"></i></div>
        <div>
            <div class="m-label">Today's Discount</div>
            <div class="m-value" style="color:#dc2626;"><?php echo formatCurrency($todayDiscount['discount']); ?></div>
            <div class="m-sub">Subtotal: <?php echo formatCurrency($todayDiscount['subtotal']); ?></div>
        </div>
    </div>
    <div class="m-stat">
        <div class="m-icon amber"><i class="fas fa-percentage"></i></div>
        <div>
            <div class="m-label">Monthly Discount</div>
            <div class="m-value" style="color:#d97706;"><?php echo formatCurrency($monthlyDiscount['discount']); ?></div>
            <div class="m-sub">Subtotal: <?php echo formatCurrency($monthlyDiscount['subtotal']); ?></div>
        </div>
    </div>
    <div class="m-stat">
    ...
    <div class="m-stat">
        ...
        <div>
            ...
            <div class="m-value"><span id="lowStockCount"><?php echo $lowStockCount; ?></span></div>
            <div class="m-sub">Products need restock</div>
        </div>
    </div>
    <div class="m-stat">
        <div class="m-icon green"><i class="fas fa-coins"></i></div>
        <div>
            <div class="m-label">Today's Profit</div>
            <div class="m-value" style="color:<?php echo $todayProfit >= 0 ? '#059669' : '#dc2626'; ?>;"><?php echo formatCurrency($todayProfit); ?></div>
            <div class="m-sub">Sales: <?php echo formatCurrency($todaySales['total']); ?> · Expenses: <?php echo formatCurrency($cashToday['cash_out']); ?></div>
        </div>
    </div>
    <div class="m-stat">
        <div class="m-icon indigo"><i class="fas fa-chart-pie"></i></div>
        <div>
            <div class="m-label">Monthly Profit</div>
            <div class="m-value" style="color:<?php echo $monthlyProfit >= 0 ? '#4f46e5' : '#dc2626'; ?>;"><?php echo formatCurrency($monthlyProfit); ?></div>
            <div class="m-sub">Sales: <?php echo formatCurrency($monthlySales['total']); ?> · Expenses: <?php echo formatCurrency($cashMonth['cash_out']); ?></div>
        </div>
    </div>
    <div class="m-stat">
        <div class="m-icon gold"><i class="fas fa-piggy-bank"></i></div>
        <div>
            <div class="m-label">All-Time Profit</div>
            <div class="m-value" style="color:<?php echo $allTimeProfit >= 0 ? '#b45309' : '#dc2626'; ?>;"><?php echo formatCurrency($allTimeProfit); ?></div>
            <div class="m-sub">Total sales minus total expenses</div>
        </div>
    </div>
</div>
                        ...
                        <?php foreach ($storeRows as $i => $ss): ?>
                            ...
                            <tr>
                                ...
                                <td style="color:#dc2626;">
                                    <strong>-<?php echo formatCurrency($ss['today_discount']); ?></strong>
                                </td>
                                <td>
                                ...
                                <td style="color:#dc2626;">
                                    <strong>-<?php echo formatCurrency($ss['month_discount']); ?></strong>
                                </td>
                                <td>
                                ...
                                <td style="color:#dc2626;">
                                    <strong>-<?php echo formatCurrency($ss['all_discount']); ?></strong>
                                </td>
                        ...
                        <?php else: ?>
                            <?php foreach ($recentSales as $sale): ?>
                                <tr>
                                    ...
                                    <td style="color:#dc2626;">-<?php echo formatCurrency($sale['discount_amount']); ?></td>
...
<!-- Platform Overview (Super Admin only - connects admin panel to frontend) -->
<?php if ($isSuper): ?>
<?php
    $platform = ['businesses' => 0, 'products' => 0, 'sales' => 0, 'revenue' => 0, 'plans' => 0, 'pending' => 0];
    try {
        $platform['businesses'] = (int)$db->query("SELECT COUNT(*) FROM stores")->fetchColumn();
        $platform['products']    = (int)$db->query("SELECT COUNT(*) FROM products")->fetchColumn();
        $platform['sales']       = (int)$db->query("SELECT COUNT(*) FROM sales")->fetchColumn();
        $platform['revenue']     = (float)$db->query("SELECT COALESCE(SUM(total),0) FROM sales")->fetchColumn();
        $platform['plans']       = (int)$db->query("SELECT COUNT(*) FROM subscription_plans WHERE status='active'")->fetchColumn();
        $platform['pending']     = (int)$db->query("SELECT COUNT(*) FROM plan_payments WHERE status='pending'")->fetchColumn();
    } catch (Exception $e) {}
?>
<div style="background:linear-gradient(135deg,#1e1b4b,#312e81 50%,#0f172a); color:#fff; border-radius:1.1rem; padding:1.5rem; margin-bottom:1.5rem;">
    <div style="display:flex; align-items:center; gap:1rem; margin-bottom:1.25rem;">
        <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#6366f1,#8b5cf6); display:flex;align-items:center;justify-content:center;font-size:1.3rem;">
            <i class="fas fa-globe"></i>
        </div>
        <div style="flex:1;">
            <div style="font-weight:800;font-size:1.1rem;">Platform Overview</div>
            <div style="font-size:0.82rem;opacity:0.8;">SaaS platform-wide stats — data shown on the public website</div>
        </div>
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
            <a href="../landing.php" target="_blank" style="color:#fff;text-decoration:none;background:rgba(255,255,255,0.15);padding:0.5rem 1rem;border-radius:999px;font-size:0.8rem;font-weight:600;">
                <i class="fas fa-external-link-alt"></i> View Website
            </a>
            <a href="plans.php" style="color:#fff;text-decoration:none;background:rgba(255,255,255,0.15);padding:0.5rem 1rem;border-radius:999px;font-size:0.8rem;font-weight:600;">
                <i class="fas fa-box-open"></i> Plans
            </a>
        </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:1rem;">
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-store"></i> Businesses</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;"><?php echo number_format($platform['businesses']); ?></div>
        </div>
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-box"></i> Products</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;"><?php echo number_format($platform['products']); ?></div>
        </div>
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-receipt"></i> Transactions</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;"><?php echo number_format($platform['sales']); ?></div>
        </div>
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-chart-line"></i> Revenue</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;"><?php echo formatCurrency($platform['revenue']); ?></div>
        </div>
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-crown"></i> Active Plans</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;"><?php echo number_format($platform['plans']); ?></div>
        </div>
        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:0.9rem;padding:0.9rem;<?php echo $platform['pending'] > 0 ? 'border-color:rgba(245,158,11,0.5);' : ''; ?>">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;opacity:0.7;"><i class="fas fa-clock"></i> Pending Approvals</div>
            <div style="font-size:1.5rem;font-weight:800;margin-top:0.25rem;<?php echo $platform['pending'] > 0 ? 'color:#fbbf24;' : ''; ?>"><?php echo number_format($platform['pending']); ?></div>
            <?php if ($platform['pending'] > 0): ?>
                <a href="review-payments.php" style="font-size:0.7rem;color:#fbbf24;text-decoration:underline;">Review now &rarr;</a>
            <?php endif; ?>
        </div>
    </div>
</div>
...
(function() {
    ...
    function refreshStockCounts() {
        fetch('api/low-stock.php', { cache: 'no-store' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                if (data && data.authenticated && data.low_stock !== undefined && countEl) {
                    var v = parseInt(data.low_stock, 10);
                    if (!isNaN(v) && v !== parseInt(countEl.textContent, 10)) {
    ...
    // Poll every 15 seconds so counts stay in sync after sales
