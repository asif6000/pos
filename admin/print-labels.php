 ...
 * @param float $targetWidthPx Target rendered width in px (96dpi CSS px); bar width auto-computed
 * @return string SVG markup
 */
function code128BarcodeSvg($data, $height = 30, $targetWidthPx = null)
{
    static $patterns = [
    ...
    $barWidth = 1.5;
    if ($targetWidthPx !== null && $totalModules > 0) {
        // Render at native size so the printer gets crisp bars (no browser scaling)
        $barWidth = min(max(((float) $targetWidthPx) / $totalModules, 0.75), 2.0);
    }
    $x = $quiet * $barWidth;
    $bars = '';
    $isBar = true;
    ...
    <style>
        ...
        * {
        ...
        .sheet {
            ...
            /* Strict grid: exact number of stickers per row, gaps between them */
            display: grid;
            grid-template-columns: repeat(<?php echo $stickersPerRow; ?>, <?php echo floatval($stickerWidth); ?>in);
            grid-auto-rows: <?php echo floatval($stickerHeight); ?>in;
            column-gap: <?php echo floatval($colDistance); ?>in;
            row-gap: <?php echo floatval($rowDistance); ?>in;
            align-content: start;
...
<body>
    ...
    $labelsPerSheet = max(1, $stickersPerSheet);
    $totalSheets = (int) ceil($totalLabels / $labelsPerSheet);
    $currentSheet = 0;

    // Processing sheets
    while ($currentLabel < $totalLabels) {
        $currentSheet++;
        // Page break only BETWEEN sheets - never after the last one (prevents blank trailing label/page)
        $isLastSheet = ($currentSheet >= $totalSheets);
        if (!$isLastSheet) {
            echo '<div class="sheet" style="height: ' . floatval($paperHeight) . 'in; overflow: hidden; page-break-after: always;">';
        } else {
                ...
                <div class="label">
                    ...
                    <?php if (!empty($product['barcode'])): ?>
                        ...
                        <div class="label-barcode">
                            <?php
                            // Usable width in CSS px (96dpi) minus label padding, so the SVG renders natively at exact size
                            $barcodeTargetPx = max(60, (int) round($stickerWidth * 96 - 16));
                            echo code128BarcodeSvg($product['barcode'], 30, $barcodeTargetPx);
                            ?>
                        </div>
