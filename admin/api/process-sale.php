...
if (!hasPermission('pos')) {
    ob_end_clean();
    echo json_encode(['success' => false, 'message' => 'Forbidden: Your plan does not include POS.']);
    exit;
}
...
try {
    ...
    if ($editSaleId > 0) {
        ...
        foreach ($oldItems as $item) {
            ...
            // Restore master product stock as well
            $db->prepare("UPDATE products SET stock = stock + ? WHERE id = ?")
                ->execute([$restoreQty, $item['product_id']]);
    ...
    foreach ($input['items'] as $item) {
        ...
        if ($existing) {
            // Deduct from existing store stock
            $db->prepare("UPDATE store_stocks SET quantity = quantity - ? WHERE store_id = ? AND product_id = ?")
        ...
        // Deduct from master product stock as well (so product.stock reflects sales)
